...
func setup_upgrades():
	available_upgrades = [
		{
			"name": "Extra Drop Zone",
			"description": "Add one more drop zone for serving customers",
			"cost": 50,
			"type": "drop_zone",
			"max_purchases": 3
		},
		{
			"name": "Faster Service",
			"description": "Customers wait 10 seconds longer before leaving",
			"cost": 30,
			"type": "patience",
			"max_purchases": 5
		},
		{
			"name": "New Item: Banana",
			"description": "Unlock banana for your shop (Circle shape)",
			"cost": 25,
			"type": "new_item",
			"item_data": {"name": "Banana", "shape": "circle", "price": 4},
			"max_purchases": 1
		},
		{
			"name": "New Item: Apple",
			"description": "Unlock apple for your shop (Circle shape)",
			"cost": 25,
			"type": "new_item",
			"item_data": {"name": "Apple", "shape": "circle", "price": 4},
			"max_purchases": 1
		},
		{
			"name": "New Item: Bread",
			"description": "Unlock bread for your shop (Rectangle shape)",
			"cost": 30,
			"type": "new_item",
			"item_data": {"name": "Bread", "shape": "rectangle", "price": 6},
			"max_purchases": 1
		},
		{
			"name": "New Item: Cookies",
			"description": "Unlock cookies for your shop (Square shape)",
			"cost": 20,
			"type": "new_item",
			"item_data": {"name": "Cookies", "shape": "square", "price": 3},
			"max_purchases": 1
		},
		{
			"name": "More Customers",
			"description": "2 more customers visit your shop each day",
			"cost": 40,
			"type": "more_customers",
			"max_purchases": 3
		},
		{
			"name": "Price Boost",
			"description": "All items sell for 1 more coin",
			"cost": 60,
			"type": "price_boost",
			"max_purchases": 2
		}
...
func show_menu(day: int, daily_earnings: int, total_money: int):
...
func apply_upgrade(upgrade: Dictionary):
	match upgrade.type:
		"drop_zone":
			...
			print("Added extra drop zone")
		
		"patience":
			# Increase customer patience
			Customer.patience_time += 10.0
			print("Increased customer patience")
		...
		"new_item":
			...
			print("Added new item: ", upgrade.item_data.name)
		
		"more_customers":
			# Increase customers per day
			var shop = get_tree().get_first_node_in_group("shop")
			if shop:
				shop.max_customers_per_day += 2
			print("Increased daily customers")
